...
export default async function AgencyMobilityRequestDetail({
  params
}: {
  ...
  if (session.user.role !== Role.SUPER_ADMIN) {
    const ids = await getCallerAgencyIds(session.user.id);
    if (!ids.includes(req.agencyId)) redirect("/agency");
  }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-600">
          {req.requestType.replace(/_/g, " ")} request • {req.bids.length} bid{req.bids.length === 1 ? "" : "s"}
        </p>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Card>
          ...
          <CardContent className="space-y-1 text-sm text-slate-700">
            <p><span className="font-medium">Name:</span> {req.candidate.name}</p>
            <p><span className="font-medium">Role:</span> {req.candidate.role} · {req.candidate.specialty}</p>
            <p><span className="font-medium">Email:</span> {req.candidate.email}</p>
            {req.candidate.phone ? <p><span className="font-medium">Phone:</span> {req.candidate.phone}</p> : null}
          </CardContent>
        </Card>
      </div>
      ...
      <div>
        ...
        <p className="text-sm text-slate-600">Sorted by concierge pick then price.</p>
      </div>

      {bids.length === 0 ? (
        <Card>
          <CardContent className="p-8 text-center text-sm text-slate-500">
            No bids received yet. Verified vendors usually respond within 2 hours.
          </CardContent>
        </Card>
      ) : (
            ...
            <BidCard
              ...
              actions={
                bid.status === "SUBMITTED" || bid.status === "SHORTLISTED" ? (
                  <AcceptBidButton bidId={bid.id} paymentResponsibility="AGENCY" label="Accept (agency-funded)" />
                ) : null
              }
            />
          ))}
